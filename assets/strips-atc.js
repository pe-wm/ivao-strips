(function () {
	...
	function createStrip(stage, cfgType, cfg, withTransferBtn, onTransfer, saveFn, endpoints, onDuplicate) {
		...
		if (withTransferBtn) {
			...
			if (typeof onDuplicate === 'function') {
				...
				btnDup.type = 'button';
				btnDup.className = 'se-btn';
				btnDup.textContent = 'Duplicar';
				btnDup.style.position = 'absolute'; btnDup.style.right = '-106px'; btnDup.style.bottom = '50px';
				btnDup.addEventListener('click', () => { onDuplicate(layer); });
				layer.appendChild(btnDup);
			}
		}
	...
	function initATCInstance(root) {
		...
		// 3) Reloj UTC
		// 3) Reloj UTC y Custom
		if (elClock) {
			const pad = n => n < 10 ? '0' + n : '' + n;

			// === Custom Clock Setup ===
			let customTimeOffset = 0; // ms offset from real time

			// Create container for custom clock
			const customBox = document.createElement('span');
			customBox.style.marginLeft = '20px';
			customBox.style.display = 'inline-flex';
			customBox.style.alignItems = 'center';
			customBox.style.gap = '10px';

			const customLabel = document.createElement('span');
			customLabel.textContent = 'Hora personalizada: ';
			customLabel.style.fontWeight = 'bold';

			const elCustomClock = document.createElement('div');
			elCustomClock.style.color = 'yellow';
			elCustomClock.style.fontWeight = 'bold';
			//elCustomClock.style.fontSize = '1.1em';
			elCustomClock.textContent = '--:--:--';
			elCustomClock.className = 'se-clock';

			const btnChangeTime = document.createElement('button');
			btnChangeTime.type = 'button';
			btnChangeTime.className = 'se-btn'; // reuse existing btn class
			btnChangeTime.textContent = 'Cambiar';
			//btnChangeTime.style.fontSize = '0.8em';
			//btnChangeTime.style.padding = '2px 8px';

			const btnPause = document.createElement('button');
			btnPause.type = 'button';
			btnPause.className = 'se-btn';
			btnPause.textContent = 'Pausar';
			btnPause.style.marginLeft = '5px';

			customBox.appendChild(customLabel);
			customBox.appendChild(elCustomClock);
			customBox.appendChild(btnChangeTime);
			customBox.appendChild(btnPause);

			// Insert after the original clock's parent (horaBox) or append to it?
			// The existing code does: const horaBox = elClock.parentNode;
			// Let's append to horaBox to keep it on the same line if possible
			if (elClock.parentNode) {
				elClock.parentNode.appendChild(customBox);
			}

			// Button Logic
			btnChangeTime.addEventListener('click', () => {
				const input = prompt('Introduce la nueva hora (HH:MM:SS)', elCustomClock.textContent);
				if (!input) return;
				...
				// Create date object for "today" at that time
				const now = new Date();
				const target = new Date(now.getFullYear(), now.getMonth(), now.getDate(), h, m, s);

				// Calculate offset: target - now
				// We want displayed_time = real_time + offset
				// So offset = target - real_time
				customTimeOffset = target.getTime() - now.getTime();

				// If we were paused, unpause and set state to running from new time
				if (isPaused) {
					isPaused = false;
					pausedTime = null;
					btnPause.textContent = 'Pausar';
			...
			btnPause.addEventListener('click', () => {
				isPaused = !isPaused;
				if (isPaused) {
					// Pause: capture current custom time
					const now = new Date();
					pausedTime = new Date(now.getTime() + customTimeOffset);
					btnPause.textContent = 'Reanudar';
					btnPause.style.backgroundColor = '#e74c3c'; // Red-ish to indicate paused/attention
				} else {
					// Resume: calculate new offset to continue from pausedTime
					// new_offset = pausedTime - now
					const now = new Date();
					customTimeOffset = pausedTime.getTime() - now.getTime();
					pausedTime = null;
					btnPause.textContent = 'Pausar';
					btnPause.style.backgroundColor = ''; // Reset
				}
				tick();
			});
			...
			function tick() {
				...
				if (isPaused && pausedTime) {
					customDate = pausedTime;
				} else {
					customDate = new Date(now.getTime() + customTimeOffset);
				}

				const chh = pad(customDate.getHours());
				const cmm = pad(customDate.getMinutes());
				const css = pad(customDate.getSeconds());
				elCustomClock.textContent = `${chh}:${cmm}:${css}`;
			}
			tick(); setInterval(tick, 1000);
			console.info('[strips-atc] relojes iniciados');
		} else {
		...
		function addStrip(type, opts) {
			const layer = createStrip(
				...
				(originLayer) => {
					// Duplicar strip
					const data = serializeStrip(originLayer);
					const newLayer = addStrip(type, { atTop: true });
					applyStripToLayer(newLayer, data);
					saveNow(endpoints, stage);
				}
			);
